...
import { editPost, getPostById } from "../controllers/Post.controller";
...
const EditPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const { resp, isSame } = res.data.data;
        ...
        setSelectedType(resp.type);
        setTags(resp.tags);
        setLocation(resp.location);

        setExistingImages(resp.images); // URLs
        setImagePreviews(resp.images); // show previews
      } catch (err) {
  ...
  const removeImage = (index) => {
    const isOldImage = index < existingImages.length;
